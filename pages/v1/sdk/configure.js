				...
				<Card>
					<div className="flex flex-col w-full max-w-lg max-h-[80vh]">
						...
						<div className="p-6 space-y-6 overflow-y-auto">
							<div>
								...
								<Text>
									...
									website and use its features. Without these cookies services
							...
							<div>
								<div className="flex items-center justify-between mb-4">
									...
									<Toggle
										id="statistical-toggle"
										active={config.statistical}
										onChange={(active) =>
											setConfig({ ...config, statistical: active })
										}
									/>
								</div>
								<Text>
									These cookies track user behaviour on the website such as
									pages visited, duration of time spent on a page and what users
									are doing on a page.
								</Text>
							</div>
							<div>
								<div className="flex items-center justify-between mb-4">
									...
									<Toggle
										id="marketing-toggle"
										active={config.marketing}
										onChange={(active) =>
											setConfig({ ...config, marketing: active })
										}
									/>
								</div>
								<Text>
									These cookies are set through our site by our advertising
									partners which can help identify users in order to show
									relevant material.
								</Text>
							</div>
							<div>
								<div className="flex items-center justify-between mb-4">
									...
									<Toggle
										...
										active={config.preference}
										onChange={(active) =>
											setConfig({ ...config, preference: active })
										}
									/>
								</div>
								<Text>
									These cookies are used to remember choices {`you've`} made on
									the site in order to personalise and persist your experience
						...
						<div className="flex p-6">
							<Button id="save-trigger" color="black" block onClick={onSave}>
								Save
							</Button>
						</div>
					</div>
